...
func login(c *gin.Context) {
	...
	// ตั้ง cookie สำหรับ access token และ refresh token
	c.SetCookie("access_token", accessToken, 15*60, "/", "localhost", false, true)   // 15 นาที
	c.SetCookie("refresh_token", refreshToken, 7*24*60*60, "/", "localhost", false, true) // 7 วัน
	...
	// ส่ง response JSON (ยังคงเหมือนเดิม)
...
func refreshTokenHandler(c *gin.Context) {
    ...
    if err != nil {
        c.JSON(http.StatusBadRequest, gin.H{"error": "refresh token cookie missing"})
        return
    }

    userID, valid := isRefreshTokenValid(token)
    if !valid {
...
func logout(c *gin.Context) {
	...
	// ลบ cookies ของ access และ refresh token
...
func authMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// 2. ถ้า header ไม่มี ลองดึงจาก cookie
		if tokenString == "" {
